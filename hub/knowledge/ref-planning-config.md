...
| `workflow.subagent_timeout` | `300000` | ⚠️ **已废弃（PolarCopilot 禁止使用 subagent）**。仅为 gsd-1 兼容保留，PolarCopilot 中所有 Agent 都是独立 Cursor CLI session，不使用 subagent。|
</config_schema>
...
**Auto-detection:** If `.planning/` is gitignored, `commit_docs` is effectively `false`. Check via:

```bash
git check-ignore .planning/ && echo "gitignored" || echo "tracked"
```
...
Read `.planning/config.json` directly for `branching_strategy`, `phase_branch_template`, `milestone_branch_template`.
