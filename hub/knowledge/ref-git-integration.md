...
- ⛔ **绝对禁止** 创建 public 仓库
- 必须显式使用 `--private` 参数
- 命令：`gh repo create <name> --private --source=. --remote=origin --push`
- 禁止省略 `--private`（默认可能是 public）
- 如果发现仓库是 public，立即执行 `gh repo edit <owner>/<repo> --visibility private`
...
1. **Auto-detection:** During `$gsd2-new-project`, directories with their own `.git` folder are detected and offered for selection as sub-repos.
2. **File grouping:** Code files are grouped by their sub-repo prefix (e.g., `backend/src/api/users.ts` belongs to the `backend/` repo).
3. **Independent commits:** Each sub-repo receives its own atomic commit. File paths are made relative to the sub-repo root before staging.
...
```bash
# Stage files per sub-repo and commit independently:
cd backend && git add src/api/users.ts src/types/user.ts && git commit -m "feat(02-01): add user API" && cd ..
cd frontend && git add src/components/UserForm.tsx && git commit -m "feat(02-01): add user API" && cd ..
```
